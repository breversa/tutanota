...
final class CredentialsFacadeTest: XCTestCase {
	...
	let encryptedCredentials1 = PersistedCredentials(
	...
	let decryptedCredentials1 = UnencryptedCredentials(
		credentialInfo: CredentialsInfo(login: "login1@test.com", userId: "user1", type: CredentialType._internal),
	...
	let encryptedCredentials2 = PersistedCredentials(
	...
	let encCredentialsKey = Data([0x0e])
	let decCredentialsKey = Data([0x0d])
	let reEncryptedCredentialsKey = Data([0x03])
	...
	func test_GetCredentialEncryptionode_returns_null_from_the_db() async throws {
		...
		try await XCTAssertEqual(nil, facade.getCredentialEncryptionMode())
	}
	...
	func test_loadByUserId_$_when_there_is_a_key_it_is_used_to_decrypt_credentials() async throws {
		given(credentialsDb.getCredentialsEncryptionKey()).willReturn(encCredentialsKey)
	...
	func test_loadByUserId_$_when_another_mode_is_selected_the_migration_is_done() async throws {
		...
		let loadedCredential = try await facade.loadByUserId("user1")
		XCTAssertEqual(decryptedCredentials1, loadedCredential)
		verify(credentialsDb.setCredentialsEncryptionKey(encryptionKey: reEncryptedCredentialsKey)).wasCalled()
	}

	func test_store_$_when_there_is_no_key_it_generates_and_stores_one() async throws {
		given(credentialsDb.getCredentialsEncryptionKey()).willReturn(nil)
	...
	func test_migrate_stores_everything() async throws {
		...
		verify(credentialsDb.setCredentialsEncryptionKey(encryptionKey: encCredentialsKey)).wasCalled()
