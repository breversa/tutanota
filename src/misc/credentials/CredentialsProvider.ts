...
export class CredentialsProvider {
	...
	async getCredentialsInfoByUserId(userId: Id): Promise<CredentialsInfo | null> {
		...
		return PersistedCredentials?.credentialInfo ?? null
	}
	...
	async getInternalCredentialsInfos(): Promise<ReadonlyArray<CredentialsInfo>> {
		const allCredentials = (await this.storage.loadAll()).map((persistedCredentials) => persistedCredentials.credentialInfo)
