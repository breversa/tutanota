...
public enum TaggedSqlValue: Codable {
	...
	public init(from decoder: Decoder) throws {
		let typeString = try decoder.container(keyedBy: Self.CodingKeys.self).decode(String.self, forKey: .type)
		guard let type = SqlType(rawValue: typeString) else { fatalError("unknown sql type \(typeString), can't decode") }
		switch type {
		case .null: self = .null
		case .string:
			let value: String = try decoder.container(keyedBy: Self.CodingKeys.self).decode(String.self, forKey: .value)
			self = .string(value: value)
		case .number:
			let value: Int = try decoder.container(keyedBy: Self.CodingKeys.self).decode(Int.self, forKey: .value)
			self = .number(value: value)
		case .bytes:
			let value: DataWrapper = try decoder.container(keyedBy: Self.CodingKeys.self).decode(DataWrapper.self, forKey: .value)
